...
private:
    ...
    void setCommandType(const string& command) {
        ...
        else if(command == "label") command_type_ = CommandType::C_LABEL;
        else if(command == "goto") command_type_ = CommandType::C_GOTO;
        else if(command == "if-goto") command_type_ = CommandType::C_IF;
        else if(command == "function") command_type_ = CommandType::C_FUNCTION;
        else if(command == "call") command_type_ = CommandType::C_CALL;
        else if(command == "return") command_type_ = CommandType::C_RETURN;
        else command_type_ = CommandType::C_ARITHMETIC;
    }

    void setArgs(const vector<string>& commands) {
        if(command_type_ == CommandType::C_ARITHMETIC || command_type_ == CommandType::C_RETURN) {
            arg1_ = commands[0];
        } else if(command_type_ == CommandType::C_LABEL || command_type_ == CommandType::C_GOTO || command_type_ == CommandType::C_IF) {
            arg1_ = commands[1];
        } else {
            arg1_ = commands[1];
